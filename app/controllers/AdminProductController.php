...
class AdminProductController extends Controller
{
    ...
    public function create()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            //escapeshellcmd(), addslashes(), htmlentities()
            if (empty($name)) {
            ...
            if ( ! Validate::date($published)) {
                array_push($errors, 'El formato de fecha no sigue el patrón establecido');
            ...
            if ($image) {
                if (Validate::imageFile($_FILES['image']['tmp_name'])){
                    ...
                    if (is_uploaded_file($_FILES['image']['tmp_name'])) {
                        ...
                        array_push($errors, 'Error durante la subida del fichero');
                    }
                } else {
                    array_push($errors, 'El formato de la imagen no es el adecuado');
                }
            } else {
            ...
            if (empty($errors)) {
                ...
                array_push($errors, 'Se ha producido un error durante la insercion en la BD');
            }
        }
    ...
    public function update($id)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($errors)) {
                ...
                array_push($errors, 'Se ha producido un error durante la modificación en la BD');
            }
        }
        ...
        $data = [
            'titulo'    => 'Administración de Productos - Actualización',
            'menu'      => false,
            'admin'     => true,
            'type'      => $typeConfig,
            'status'    => $statusConfig,

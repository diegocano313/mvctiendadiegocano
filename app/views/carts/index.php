...
<form action="<?= ROOT ?>cart/update" method="POST">
    <table class="table table-stripped" width="100%">
        ...
        <tr>
            ...
            <td class="text-right"><input type="number" name="c<?= $key ?>" class="text-right"
                value="<?= number_format($value->quantity, 0) ?>" min="1" max="99">
                <input type="hidden" name="i<?= $key ?>" value="<?= $value->product ?>"></td>
        ...
        <?php $subtotal += $value->price * $value->quantity; $discount += $value->discount;
            $send += $value->send ?>
        <?php endforeach ?>
    ...
    <table width="100%" class="text-right">
        ...
        <tr>
            ...
            <td>
                <input type="submit" class="btn btn-info" role="button" value="Recalcular">
            </td>
            <?php if($verify): ?>
            <td>
